      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            className="w-full max-w-md overflow-hidden rounded-2xl border border-purple-900/45 bg-[#0a0a10] p-6.5 shadow-2xl relative"
          >
            {/* Close absolute */}
            <button
              id="close-lead-modal-btn"
              onClick={() => setIsOpen(false)}
              className="absolute top-4 right-4 rounded-lg bg-purple-950/20 p-1.5 text-gray-500 hover:text-white transition-colors cursor-pointer"
            >
            ...
            {!submitted ? (
              <div id="lead-form-overlay">
                <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-purple-950/40 border border-purple-500/20 text-purple-400 mb-5 relative">
                ...
                <h3 className="text-lg font-black text-white leading-snug">
                  ...
                  <span className="bg-gradient-to-r from-amber-300 via-purple-400 to-indigo-400 bg-clip-text text-transparent">AI automation Toolkit (PDF)</span>
                </h3>
                ...
                <form onSubmit={handleLeadSubmit} className="mt-5 space-y-3.5">
                  <div className="space-y-1">
                    <label className="text-[10px] uppercase tracking-wider text-gray-400 font-bold block">First Name</label>
                    <input
                      type="text"
                      required
                      value={name}
                      ...
                      className="w-full text-xs rounded-xl border border-purple-900/30 bg-[#0d0d15] px-4 py-3 text-white placeholder-gray-650 focus:border-purple-500 focus:outline-none"
                    />
                  </div>

                  <div className="space-y-1">
                    <label className="text-[10px] uppercase tracking-wider text-gray-400 font-bold block">Your Best Email address</label>
                    <input
                      type="email"
                      required
                      value={email}
                      ...
                      className="w-full text-xs rounded-xl border border-purple-900/30 bg-[#0d0d15] px-4 py-3 text-white placeholder-gray-655 focus:border-purple-500 focus:outline-none"
                    />
                  </div>

                  {errorMsg && (
                    <p className="text-[11px] text-red-400 text-center bg-red-500/5 py-1.5 rounded">{errorMsg}</p>
                  )}
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center space-x-2 rounded-xl bg-gradient-to-r from-purple-600 via-indigo-650 to-amber-500 py-3.5 text-xs font-bold text-white shadow-lg cursor-pointer hover:scale-[1.01] transition-all disabled:opacity-50"
                  >
                    <span>Download Instant Guide</span>
                    <Send className="h-3.5 w-3.5" />
                  </button>

                  <div className="flex items-center justify-center space-x-1.5 text-[9px] text-gray-500">
              ...
              <div className="text-center py-6" id="lead-success-overlay">
                ...
                <button
                  onClick={() => setIsOpen(false)}
                  className="mt-6 inline-flex items-center justify-center rounded-xl bg-purple-900/40 border border-purple-500/30 px-6 py-2.5 text-xs font-bold text-purple-300 hover:text-white transition-all cursor-pointer"
                >
                  Enter Site
                </button>
              </div>
